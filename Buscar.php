...
$pagination->rowCount("SELECT * FROM proyecto WHERE id LIKE '%$search%' OR Nombre LIKE '%$search%' ");
$pagination->config(3, 5);
$sql = "SELECT * FROM proyecto WHERE id LIKE '%$search%' OR Nombre LIKE '%$search%'  ORDER BY id ASC LIMIT $pagination->start_row, $pagination->max_rows";
...
{
$pagination->rowCount("SELECT * FROM proyecto");
$pagination->config(3, 6);
$sql = "SELECT * FROM proyecto ORDER BY id ASC LIMIT $pagination->start_row, $pagination->max_rows";
$query = $connection->prepare($sql);
$query->execute();
$model = array();
while($rows = $query->fetch())
...
<center><h1>Eliminar Proyecto</h1></center>
...
<input type="text" name="search" placeholder="Buscar proyecto a eliminar" class="form-control" value="<?php echo $search ?>">
