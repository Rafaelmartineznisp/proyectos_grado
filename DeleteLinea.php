...
if(isset($_POST["delete"])){
	...
	if(!is_numeric($id_inv)){
		header("location:ReadLinea.php");
	}
	else {
		$model= new Crud;
		$test= $model ->deleteFrom="linea_investigacion";
...
<br><br>
<?php if (isset($_GET["id_inv"])):?>
	<strong>Realmente quieres eliminar este proyecto: <?php echo htmlspecialchars($_GET["id_inv"]); ?></strong>
<form class="input" method="POST" action="<?php echo $_SERVER['PHP_SELF'];?>">
<input type="hidden" name="id_inv" value="<?php echo htmlspecialchars($_GET['id_inv']); ?>">
